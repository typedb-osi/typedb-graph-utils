...
- Use [`tool/set-version.js`](typescript/tool/set-version.js) to update `package.json` — e.g. `node tool/set-version.js 3.7.0`. The snapshot workflow uses the same script to stamp a snapshot-specific version (`<VERSION>-<sha7>`).
...
|--------------------|-------------------------------|------------------------------------------|
| `CLOUDSMITH_TOKEN` | `deploy-typescript-snapshot.yml`     | Cloudsmith npm publish auth (snapshots)  |
| `NPM_TOKEN`        | `deploy-typescript-release.yml`      | npmjs.org publish auth (releases)        |
| `GITHUB_TOKEN`     | `deploy-typescript-release.yml`      | Automatic; pushes tag and creates GH Release |
